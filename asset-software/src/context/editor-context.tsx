...
export function EditorProvider({ children }: { children: React.ReactNode }) {
  ...
  const updateObject = useCallback((updatedObj: THREE.Object3D) => {
    setObjects((prev) => {
      ...
      return prev.map((obj) =>
        obj.uuid === updatedObj.uuid ? updatedObj : obj,
      );
    });
  }, []);
  ...
  const snapObjects = useCallback(() => {
    ...
    for (const obj of objects) {
      ...
      const MARGIN = 0.1;
      const overlapX =
        movingBox.min.x < targetBox.max.x + MARGIN &&
        movingBox.max.x > targetBox.min.x - MARGIN;
      const overlapY =
        movingBox.min.y < targetBox.max.y + MARGIN &&
        movingBox.max.y > targetBox.min.y - MARGIN;
      const overlapZ =
        movingBox.min.z < targetBox.max.z + MARGIN &&
        movingBox.max.z > targetBox.min.z - MARGIN;

      // Snap X (seulement si aligné en Y et Z)
      if (overlapY && overlapZ) {
        // Face à face
        const dx1 = targetBox.min.x - movingBox.max.x;
        if (Math.abs(dx1) < bestX.dist)
          bestX = { dist: Math.abs(dx1), pos: oldWorldPos.x + dx1 };

        const dx2 = targetBox.max.x - movingBox.min.x;
        if (Math.abs(dx2) < bestX.dist)
          bestX = { dist: Math.abs(dx2), pos: oldWorldPos.x + dx2 };

        // Centre à centre
        const dx3 = targetCenter.x - movingCenter.x;
        if (Math.abs(dx3) < bestX.dist)
          bestX = { dist: Math.abs(dx3), pos: oldWorldPos.x + dx3 };
      }

      // Snap Y (seulement si aligné en X et Z)
      if (overlapX && overlapZ) {
        const dy1 = targetBox.min.y - movingBox.max.y;
        if (Math.abs(dy1) < bestY.dist)
          bestY = { dist: Math.abs(dy1), pos: oldWorldPos.y + dy1 };

        const dy2 = targetBox.max.y - movingBox.min.y;
        if (Math.abs(dy2) < bestY.dist)
          bestY = { dist: Math.abs(dy2), pos: oldWorldPos.y + dy2 };

        const dy3 = targetCenter.y - movingCenter.y;
        if (Math.abs(dy3) < bestY.dist)
          bestY = { dist: Math.abs(dy3), pos: oldWorldPos.y + dy3 };
      }

      // Snap Z (seulement si aligné en X et Y)
      if (overlapX && overlapY) {
        const dz1 = targetBox.min.z - movingBox.max.z;
        if (Math.abs(dz1) < bestZ.dist)
          bestZ = { dist: Math.abs(dz1), pos: oldWorldPos.z + dz1 };

        const dz2 = targetBox.max.z - movingBox.min.z;
        if (Math.abs(dz2) < bestZ.dist)
          bestZ = { dist: Math.abs(dz2), pos: oldWorldPos.z + dz2 };

        const dz3 = targetCenter.z - movingCenter.z;
        if (Math.abs(dz3) < bestZ.dist)
          bestZ = { dist: Math.abs(dz3), pos: oldWorldPos.z + dz3 };
      }
    }
